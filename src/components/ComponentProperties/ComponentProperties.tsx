...
export function ComponentProperties() {
	...
	const handleFlipHorizontal = () => {
		if (node && dataComponent) {
			...
			setDataComponent({
				...dataComponent,
				flip: { ...dataComponent.flip, x: newFlipX },
			});
	...
	const handleFlipVertical = () => {
		if (node && dataComponent) {
			...
			setDataComponent({
				...dataComponent,
				flip: { ...dataComponent.flip, y: newFlipY },
			});
	...
	const handleRemoveNode = () => {
		if (isSingleNodeSelection) {
			const nodeEdges = getEdges().filter(
	...
	const handleChangeState = (e: CheckboxChangeEvent) => {
		...
		if (node && dataComponent) {
			...
			setDataComponent({
				...dataComponent,
				state: { ...dataComponent.state, on: newValue },
			});
		}
	};
	...
	const handleAlign = (
		...
		switch (align) {
					...
					if (posX && nodeHeight) {
						...
						nodes.forEach((currentNode) => {
							updateNode(currentNode.id, (prevNode) => ({
								...prevNode,
								position: { ...prevNode.position, x: alignPosX },
							}));
						});
					}
				}
			...
			case "h-center":
				{
					const sortNodes = [...nodes].sort(
						(a, b) => (b?.measured?.width ?? 0) - (a?.measured?.width ?? 0)
					);
						...
						nodes.forEach((currentNode) => {
							if (currentNode.id !== highestNode.id) {
								updateNode(currentNode.id, (prevNode) => ({
									...
									position: {
										...prevNode.position,
										x: alignPosX - (currentNode.measured?.width ?? 0) / 2,
									},
								}));
							}
						});
			...
			case "v-top":
				{
					...
					if (posY && nodeHeight) {
						...
						nodes.forEach((currentNode) => {
							updateNode(currentNode.id, (prevNode) => ({
								...prevNode,
								position: { ...prevNode.position, y: alignPosY },
							}));
						});
					}
				}
			...
			case "v-center":
				{
					const sortNodes = [...nodes].sort(
						(a, b) => (b?.measured?.height ?? 0) - (a?.measured?.height ?? 0)
					);
					const highestNode = sortNodes[0];
						...
						nodes.forEach((currentNode) => {
							if (currentNode.id !== highestNode.id) {
								updateNode(currentNode.id, (prevNode) => ({
									...
									position: {
										...prevNode.position,
										y: alignPosY - (currentNode.measured?.height ?? 0) / 2,
									},
								}));
							}
						});
				...
				<>
					...
					<div className={styles.value}>
						...
						<label className={`${styles.value_label} ${styles.value_hidden}`}>
							Hidden
							<Checkbox
								onChange={handleChangeHiddenDesignator}
								checked={!dataComponent?.isDesignatorVisible}
							/>
						</label>
					</div>
									...
									<div className={styles.value}>
										<Flex vertical>
											...
											<Input
												...
												addonAfter={
													dataComponent && (
														<Select
															defaultValue={dataComponent.prefix}
															value={dataComponent.prefix}
										...
										<label className={`${styles.value_label} ${styles.value_hidden}`}>
											...
											<Checkbox
												...
												checked={!dataComponent?.isValueVisible}
											/>
										</label>
									</div>
											...
											<div className={styles.value}>
												<Flex vertical>
													...
													<Input
														...
														addonAfter={
															dataComponent && (
																<Select
																	defaultValue={dataComponent.prefix_optional}
																	value={dataComponent.prefix_optional}
												...
												<label className={`${styles.value_label} ${styles.value_hidden}`}>
													...
													<Checkbox
														...
														checked={!dataComponent?.isValueOptionalVisible}
													/>
												</label>
											</div>
										</>
								...
								<>
									...
									<Flex vertical>
										...
										<Flex vertical align="flex-start" justify="center">
											<ColorPicker
												presets={presets}
												value={dataComponent?.color}
												styles={{ popupOverlayInner: { maxWidth: "85px" } }}
												onChange={handleChangeColorWire}
												format="hex"
												disabledFormat
											/>
										</Flex>
						...
						{selectedNodes?.map((node) => (
							<Tooltip key={node.id} placement="top" color="cyan" title={node.data?.designator}>
								<Button
									...
									onClick={() => handleClickSelectedNode(node)}
								>
									{" "}
									{`${node.data?.name}-${node.data.designator}`}{" "}
								</Button>
							</Tooltip>
						))}
					...
					<Flex gap={10} wrap>
						<Tooltip placement="top" title="Flip Horizontal (Ctrl+Alt+Left)" color="cyan">
							...
							>
								<FlipHIcon
									...
									style={{ transform: `scaleX(${dataComponent?.flip.x})` }}
								/>
							</Button>
						</Tooltip>
						<Tooltip placement="top" title="Flip Vertical (Ctrl+Alt+Down)" color="cyan">
							...
							>
								<FlipVIcon
									className={styles.button_icon}
									style={{ transform: `scaleY(${dataComponent?.flip.y})` }}
								/>
							</Button>
						</Tooltip>
