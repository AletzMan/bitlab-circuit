...
export function SwitchSPDT({ data: { type, rotation, flip, collapsed, isLock, reference, isReferenceVisible, connectedHandles, size, state }, selected, id, parentId }: NodeProps<AnalogNode>) {
    ...
    const positionTerminals: { position: Position[], adjustment: CSSProperties[] } = useMemo(() => {
        ...
        switch (rotation) {
            case 0: {
                ...
                adjustment = [{ top: `calc(50% )` }, { top: `calc(50% + 15px)` }, { top: `calc(50% - 15px)` }];
            ...
            case 90: {
                ...
                adjustment = [{ left: `calc(50%)` }, { left: `calc(50% - 15px)` }, { left: `calc(50% + 15px)` }];
                if (flip.x === -1 && flip.y === 1) {
                    tempRotation = [Position.Top, Position.Bottom, Position.Bottom];
                    adjustment = [{ left: `calc(50%)` }, { left: `calc(50% + 15px)` }, { left: `calc(50% - 15px)` }];
                } else if (flip.y === -1 && flip.x === 1) {
