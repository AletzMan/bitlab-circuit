...
export function LogicGate({ data: { type, rotation, flip, collapsed, isLock, designator, isDesignatorVisible, connectedHandles, size }, selected, id, parentId }: NodeProps<AnalogNode>) {
    ...
    const terminalSettings: ITerminalSetting = useMemo(() => {
        ...
        const positionPIN1 = (type === ComponentType.NOTGate || type === ComponentType.BUFFERGate) ? "0" : "10px";
        switch (rotation) {
            case 0: {
                position = [Position.Left, Position.Right, Position.Left];
                adjustment = [{ top: `calc(50% + ${positionPIN1})` }, { top: "50%" }, { top: "calc(50% - 10px)" }];
            ...
            case 90: {
                ...
                adjustment = [{ left: `calc(50% - ${positionPIN1})` }, { left: "50%" }, { left: "calc(50% + 10px)" }];
                if (flip.x === -1 && flip.y === 1) {
                    position = [Position.Top, Position.Bottom, Position.Top];
                    adjustment = [{ left: `calc(50% + ${positionPIN1})` }, { left: "50%" }, { left: "calc(50% - 10px)" }];
                } else if (flip.y === -1 && flip.x === 1) {
    ...
    return (
        <div className={`${styles.box}  ${isAdditionValid && styles.box_valid} ${isAdditionInvalid && styles.box_invalid}`} >
            ...
            {(type !== ComponentType.NOTGate && type !== ComponentType.BUFFERGate) && <Terminal type="source" position={terminalSettings.position[2]} id="3" isConnectable={!isConnected[2]} style={terminalSettings.adjustment[2]} />}
