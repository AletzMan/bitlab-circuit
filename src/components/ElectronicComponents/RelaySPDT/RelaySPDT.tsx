...
export function RelaySPDT({ data: { type, rotation, flip, collapsed, isLock, designator, isDesignatorVisible, connectedHandles, size, state }, selected, id, parentId }: NodeProps<AnalogNode>) {
    ...
    const terminalSettings: { position: Position[], adjustment: CSSProperties[] } = useMemo(() => {
        ...
        switch (rotation) {
            case 0: {
                position = [Position.Left, Position.Left, Position.Right, Position.Right, Position.Right];
                adjustment = [{ top: `calc(50% + 20px)` }, { top: `calc(50% - 10px)` }, { top: `calc(50% - 20px)` }, { top: `50%` }, { top: `calc(50% + 20px)` }];
                if (flip.x === -1 && flip.y === 1) {
                    position = [Position.Right, Position.Right, Position.Left, Position.Left, Position.Left];
                    adjustment = [{ top: `calc(50% + 20px)` }, { top: `calc(50% - 10px)` }, { top: `calc(50% - 20px)` }, { top: `50%` }, { top: `calc(50% + 20px)` }];
            ...
            case 90: {
                position = [Position.Top, Position.Top, Position.Bottom, Position.Bottom, Position.Bottom];
                adjustment = [{ left: `calc(50% - 20px)` }, { left: `calc(50% + 10px)` }, { left: `calc(50% + 20px)` }, { left: '50%' }, { left: `calc(50% - 20px)` }];
                if (flip.x === -1 && flip.y === 1) {
                    position = [Position.Top, Position.Top, Position.Bottom, Position.Bottom, Position.Bottom];
                    adjustment = [{ left: `calc(50% + 20px)` }, { left: `calc(50% - 10px)` }, { left: `calc(50% - 20px)` }, { left: '50%' }, { left: `calc(50% + 20px)` }];
                } else if (flip.y === -1 && flip.x === 1) {
                    position = [Position.Bottom, Position.Bottom, Position.Top, Position.Top, Position.Top];
                    adjustment = [{ left: `calc(50% - 20px)` }, { left: `calc(50% + 10px)` }, { left: `calc(50% + 20px)` }, { left: '50%' }, { left: `calc(50% - 20px)` }];
                } else if (flip.y === -1 && flip.x === -1) {
                    position = [Position.Bottom, Position.Bottom, Position.Top, Position.Top, Position.Top];
                    adjustment = [{ left: `calc(50% + 20px)` }, { left: `calc(50% - 10px)` }, { left: `calc(50% - 20px)` }, { left: '50%' }, { left: `calc(50% + 20px)` }];
