...
export function PhotoTransistorComponent({
	data: {
		type,
		value,
		rotation,
	...
	const terminalSettings: { position: Position[]; adjustment: CSSProperties[] } = useMemo(() => {
		...
		switch (rotation) {
			case 0: {
				...
				adjustment = [{ left: `calc(50% + 10px)` }, { left: `calc(50% + 10px)` }];
			...
			case 90: {
				...
				adjustment = [{ top: `calc(50% + 10px)` }, { top: `calc(50% + 10px)` }];
				if (flip.x === -1 && flip.y === 1) {
			...
			default:
				return { position: [Position.Left, Position.Top], adjustment: [] };
		}
