...
export function SwitchDPST({ data: { type, rotation, flip, collapsed, isLock, reference, isReferenceVisible, connectedHandles, size, state }, selected, id, parentId }: NodeProps<AnalogNode>) {
    ...
    const positionTerminals: { position: Position[], adjustment: CSSProperties[] } = useMemo(() => {
        ...
        switch (rotation) {
            case 0: {
                tempRotation = [Position.Left, Position.Left, Position.Right, Position.Right];
                adjustment = [{ top: `calc(50% + 13px)` }, { top: `calc(50% - 13px)` }, { top: `calc(50% - 13px)` }, { top: `calc(50% + 13px)` }];
                if (flip.x === -1 && flip.y === 1) {
                    tempRotation = [Position.Right, Position.Right, Position.Left, Position.Left];
                    adjustment = [{ top: `calc(50% + 13px)` }, { top: `calc(50% - 13px)` }, { top: `calc(50% - 13px)` }, { top: `calc(50% + 13px)` }];
            ...
            case 90: {
                tempRotation = [Position.Top, Position.Top, Position.Bottom, Position.Bottom];
                adjustment = [{ left: `calc(50% - 13px)` }, { left: `calc(50% + 13px)` }, { left: `calc(50% + 13px)` }, { left: `calc(50% - 13px)` }];
                if (flip.x === -1 && flip.y === 1) {
                    tempRotation = [Position.Top, Position.Top, Position.Bottom, Position.Bottom];
                    adjustment = [{ left: `calc(50% + 13px)` }, { left: `calc(50% - 13px)` }, { left: `calc(50% - 13px)` }, { left: `calc(50% + 13px)` }];
                } else if (flip.y === -1 && flip.x === 1) {
                    tempRotation = [Position.Bottom, Position.Bottom, Position.Top, Position.Top];
                    adjustment = [{ left: `calc(50% - 13px)` }, { left: `calc(50% + 13px)` }, { left: `calc(50% + 13px)` }, { left: `calc(50% - 13px)` }];
                } else if (flip.y === -1 && flip.x === -1) {
                    tempRotation = [Position.Bottom, Position.Bottom, Position.Top, Position.Top];
                    adjustment = [{ left: `calc(50% + 13px)` }, { left: `calc(50% - 13px)` }, { left: `calc(50% - 13px)` }, { left: `calc(50% + 13px)` }];
